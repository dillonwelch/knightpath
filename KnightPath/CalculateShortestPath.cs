    ...
    public class CalculateShortestPath
    {
        ...
        public Path Run([QueueTrigger("knightpathqueue")] QueueMessage message)
        {
            try
            {
                ...
                CreateKnightPathQueueMessage? input =
                    JsonSerializer.Deserialize<CreateKnightPathQueueMessage>(message.MessageText);
                ArgumentNullException.ThrowIfNull(input);
                ValidatePosition(input.Source);
                ValidatePosition(input.Target);

                Guid trackingId = Guid.Parse(input.TrackingId);
                IList<string> shortestPath = ShortestPathCalculator.CalculateShortestPath(
                    input.Source,
                    input.Target
                );
