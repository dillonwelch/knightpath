...
public class CalculateShortestPathTest
{
    ...
    public void CalculateShortestPathSuccessTest()
    {
        ...
        Path response = new CalculateShortestPath(new NullLogger<CalculateShortestPath>()).Run(
            message
        );
    ...
    public void CalculateShortestPathMissingKeysTest()
    {
        ...
        Assert.That(
            e.Message,
            Is.EqualTo(
                "JSON deserialization for type 'KnightPath.CreateKnightPathQueueMessage' was missing required properties, including the following: TrackingId, Source, Target"
            )
        );
    }
    ...
    public void CalculateShortestPathInvalidJsonTest()
    {
        ...
        Assert.That(
            e.Message,
            Is.EqualTo(
                "'H' is an invalid start of a property name. Expected a '\"'. Path: $ | LineNumber: 0 | BytePositionInLine: 2."
            )
        );
    }
}
