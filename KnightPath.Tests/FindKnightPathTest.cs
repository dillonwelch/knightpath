...
public class FindKnightPathTest
{
    ...
    public async Task FindKnightPathSuccessTest()
    {
        ...
        HttpResponseData response = await FindKnightPath
            .RunAsync(mockHttpRequest, [path])
            .ConfigureAwait(false);

        string streamText = await ReadBody(response.Body).ConfigureAwait(false);
        string json =
            "{\"Starting\":\"A1\",\"Ending\":\"D5\",\"ShortestPath\":\"A1:C2:B4:D5\",\"NumberOfMoves\":3,\"OperationId\":\""
